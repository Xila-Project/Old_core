...
public:
    ...
    WT32_SC01_Driver_Class(void)
    {
        {                                      // バス制御の設定を行います。
            ...
            cfg.freq_read = 6000000;           // 受信時のSPIクロック
        ...
        {                                        // 表示パネル制御の設定を行います。
            ...
            cfg.memory_width = Display_Vertical_Definition;                           // ドライバICがサポートしている最大の幅
            cfg.memory_height = Display_Horizontal_Definition;                          // ドライバICがサポートしている最大の高さ
