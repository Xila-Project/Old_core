...
#include "Core/Graphics/Canvas.hpp"
...
const Class_Type &Canvas_Class::Class = lv_canvas_class;
...
{
    Auto_Semaphore_Type Semaphore = Graphics.Take_Semaphore_Auto();
    lv_canvas_fill_bg(Get_Pointer(), Color.Get_LVGL_Color(), static_cast<lv_opa_t>(Opacity));
}
...
{
    Auto_Semaphore_Type Semaphore = Graphics.Take_Semaphore_Auto();
    lv_canvas_set_px_opa(Get_Pointer(), X, Y, static_cast<lv_opa_t>(Opacity));
}
